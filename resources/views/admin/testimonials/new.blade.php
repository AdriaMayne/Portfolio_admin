...
@section('content')
    <div class="container">
        <h4 class="text-center my-4 my-md-5">Nuevo TESTIMONIAL</h4>
        @include('partial.errores')
        <form action="{{ action('TestimonialController@store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-row">
                ...
                <div class="form-group col-md-6">
                    ...
                    <select name="order" id="order" class="form-control" required>
                        @foreach ($orderNumbers as $orderNumber)
                            @if (($orderNumber == old('order')) || ($orderNumber == $lastNum))
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                    ...
                    <textarea name="message" id="message" class="form-control col-12" placeholder="Introduce el mensaje" rows="3" required>{{ old('message') }}</textarea>
                </div>
            </div>
            <div class="form-row">
                <div class="form-group col-md-12">
